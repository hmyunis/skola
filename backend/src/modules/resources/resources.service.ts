...
} from './entities/resource-report.entity';
...
export class ResourcesService {
  constructor(
    ...
    private reportRepo: Repository<ResourceReport>,
    ...
    return {
      ...
      uploader: resource.uploader
        ? {
            ...
            name: resource.uploader.deletedAt
              ? DELETED_USER_NAME
              : resource.uploader.name,
            initials: resource.uploader.deletedAt
              ? 'DU'
  ...
  private applyResourceFilters(
    ...
    if (queryDto.courseId) {
      query.andWhere('resource.courseId = :courseId', {
  ...
  async findAllForClassroom(
    ...
    return {
      data: resources.map((res) =>
        this.toResponse(res, voteMap.get(res.id) || null),
      ),
      meta: {
        total,
        page,
  ...
  async vote(
    ...
    const nextUpvotes = Math.max(0, resource.upvotes + upDelta);
    const nextDownvotes = Math.max(0, resource.downvotes + downDelta);
    await this.resourceRepo.update(
      { id: resource.id, classroomId },
      {
        upvotes: nextUpvotes,
  ...
  async deleteResource(
    ...
    const canDelete =
      ...
      (await this.canModerate(classroomId, userId));
    if (!canDelete) {
  ...
  async reportResource(
    ...
    if (existing) {
      throw new BadRequestException(
        'You already submitted a pending report for this resource',
      );
    }
  ...
  async reviewReport(
    ...
    if (
      data.status === ResourceReportStatus.RESOLVED &&
      data.removeResource &&
      report.resource
    ) {
